 ...
 * Adapted for Ext JS 2.2 (Antonio Jimenez)
...
Ext.ux.grid.RowExpander = Ext.extend( Ext.util.Observable, {
	...
	constructor: function( config){
		...
		var css =
			'.x-' + this.id + '-grid3-row-collapsed .x-grid3-row-expander { background-position:4px 2px; }' +
			'.x-' + this.id + '-grid3-row-expanded .x-grid3-row-expander { background-position:-21px 2px; }' +
			...
			'.x-grid3-body .x-grid3-td-' + this.id + ' .x-grid3-cell-inner { height: 100%; padding: 0 !important; }' +
    ...
    getRowClass : function(record, rowIndex, p, ds){
        ...
        if(this.grid.getRowClassForExpanderPlugin)
        {
	        var extraClass = this.grid.getRowClassForExpanderPlugin(record, rowIndex, p, ds);
	        if (extraClass){
	        	cssClass += ' ' + extraClass;
	        }
        }
